...
<div class="max-w-5xl mx-auto px-6 mt-16 mb-20">
    ...
    <form action="{{ route('admin.tests.update', $test) }}" method="POST" id="test-form"
        ...
        {{-- Questions list with badge style --}}
        <div id="questions_section">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <input type="checkbox" id="q_{{ $question->id }}" name="questions[]" value="{{ $question->id }}" 
                           class="peer sr-only" 
                           {{ in_array($question->id, old('questions', $selectedQuestions)) ? 'checked' : '' }} />
                    <label for="q_{{ $question->id }}" 
                           class="flex items-center p-4 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-[#2a2a2a] cursor-pointer hover:border-[#54b5ff] peer-checked:border-[#54b5ff] peer-checked:bg-[#54b5ff]/10 transition">
